...
void CPUStats::SetCoreFilter(const std::string& filter_spec) {
    m_coreFilter = CoreFilter();

    if (filter_spec.empty() || filter_spec == "all") {
        m_coreFilter.type = CoreFilter::SHOW_ALL;
        return;
    }

    if (filter_spec.find('+') != std::string::npos) {
        m_coreFilter.type = CoreFilter::SHOW_COMBINED;

        std::stringstream ss(filter_spec);
        std::string group_spec;

        while (std::getline(ss, group_spec, ',')) {
            CoreGroup group;
            std::stringstream group_ss(group_spec);
            std::string core_spec;

            while (std::getline(group_ss, core_spec, '+')) {
                if (core_spec.find('-') != std::string::npos) {
                    size_t dash_pos = core_spec.find('-');
                    int start = std::stoi(core_spec.substr(0, dash_pos));
                    int end = std::stoi(core_spec.substr(dash_pos + 1));
                    for (int i = start; i <= end; i++) {
                        group.core_ids.push_back(i);
                    }
                } else {
                    group.core_ids.push_back(std::stoi(core_spec));
                }
            }

            if (group.core_ids.size() == 1) {
                group.display_name = std::to_string(group.core_ids[0]);
            } else {
                group.display_name = "";
                for (size_t i = 0; i < group.core_ids.size(); i++) {
                    if (i > 0) group.display_name += "+";
                    group.display_name += std::to_string(group.core_ids[i]);
                }
            }

            m_coreFilter.core_groups.push_back(group);
        }
    } else {
        m_coreFilter.type = CoreFilter::SHOW_RANGE;
        ...
        while (std::getline(ss, part, ',')) {
            if (part.find('-') != std::string::npos) {
                ...
                for (int i = start; i <= end; i++) {
                    m_coreFilter.individual_cores.push_back(i);
                }
            } else {
                m_coreFilter.individual_cores.push_back(std::stoi(part));
            }
        }
    }
}

std::vector<CoreGroup> CPUStats::GetFilteredCores() {
    std::vector<CoreGroup> result;

    if (m_coreFilter.type == CoreFilter::SHOW_COMBINED) {
        for (auto group : m_coreFilter.core_groups) {
            std::vector<float> loads;
            std::vector<int> freqs;

            for (int core_id : group.core_ids) {
                if (core_id < (int)m_cpuData.size()) {
                    loads.push_back(m_cpuData[core_id].percent);
                    freqs.push_back(m_cpuData[core_id].mhz);
                }
            }
            ...
            if (!loads.empty()) {
                ...
                group.combined_load = loads[loads.size() / 2];
                group.combined_freq = freqs[freqs.size() / 2];

                if (group.core_ids[0] < (int)m_cpuData.size()) {
                    group.is_performance_core = (m_cpuData[group.core_ids[0]].label == "P");
                }

                result.push_back(group);
            }
        }
    } else if (m_coreFilter.type == CoreFilter::SHOW_RANGE) {
        for (int core_id : m_coreFilter.individual_cores) {
            if (core_id < (int)m_cpuData.size()) {
                CoreGroup group;
                group.core_ids.push_back(core_id);
                group.combined_load = m_cpuData[core_id].percent;
                group.combined_freq = m_cpuData[core_id].mhz;
                group.display_name = std::to_string(core_id);
                group.is_performance_core = (m_cpuData[core_id].label == "P");
                result.push_back(group);
            }
        }
    } else {
        for (size_t i = 0; i < m_cpuData.size(); i++) {
            CoreGroup group;
            group.core_ids.push_back(m_cpuData[i].cpu_id);
            group.combined_load = m_cpuData[i].percent;
            group.combined_freq = m_cpuData[i].mhz;
            group.display_name = std::to_string(m_cpuData[i].cpu_id);
            group.is_performance_core = (m_cpuData[i].label == "P");
            result.push_back(group);
        }
    }
